...
namespace Core;
    ...
    class S_DATABASE
    {
        ...
        private static function getConnection()
        {
            if (is_null(self::$PDO))
            {
                try
                {
                    ...
                    self::$PDO = new PDO($dns, S_CONFIG::getConfig('DBUser'), S_CONFIG::getConfig('DBPswd'), $opt);
        ...
        {
            self::$stmt = self::getConnection()->prepare($query);
            ...
            if (self::$stmt)
            {
                ...
                if (strcmp($statement,'select') == 0)
                {
                    self::$lastQuery = $query;
                    return self::$stmt;
                } 
                else if (in_array ($statement, ['insert', 'update', 'delete'])) {
                    return [self::$stmt->rowCount(), self::$stmt->error];
                }
            }
